...
public class Simulatore {
	...
	public Simulatore(Graph<Team, DefaultWeightedEdge> grafo, Model model, Map<Integer, Team> idMapTeams) {
		super();
		this.grafo = grafo;
		this.model = model;
	...
	private void processEvent(Event e) {
		...
		case PROMOZIONE:
			...
			if(this.stato.get(teamScelto) > 0) {
				List<Adiacenza> migliori = this.model.squadreMigliori(teamScelto);
				if(!migliori.isEmpty()) {
		...
		case BOCCIATURA:
			...
			if(this.stato.get(teamScelto) > 0) {
				List<Adiacenza> peggiori = this.model.squadrePeggiori(teamScelto);
				if(!peggiori.isEmpty()) {
